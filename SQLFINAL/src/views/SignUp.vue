<template>
  ...
  <form class="sign">
    ...
    <div class="field">
    <button type="button" class="create" v-if="samePassword() && this.password != ''" @click.prevent="createUser">Create</button>
    <button type="button" class="create" v-else @click.prevent="createUser" disabled title="Please fill out all the fields above">Create</button>
    </div>
  </form>
  <div id="result"></div>
...
=======
      ...
      if (emailExists.data.length > 0) {
        ...
        if (signUpData.error) {
          ...
          alert("We have sent a verification email to you. Please click the link to verify your account.")
          const signUpData = await supabase.auth.signUp({
          email: this.email,
          password: this.password
        })
